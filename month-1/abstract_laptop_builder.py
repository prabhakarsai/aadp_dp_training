...
class dellBuilder(Builder):
    ...
    def getRAMMemory(self): 
        ram_memory = RAMMemory()
        ram_memory.memory = '3 GB'
        return ram_memory

    def getDiskMemory(self): 
        disk_memory = DiskMemory()
        disk_memory.memory = '500 GB'
        return disk_memory

    def getScreenSize(self): 
        screen_size = ScreenSize()
        screen_size.size = "15 inch"
        return screen_size
...
def main(laptop_name):
    if laptop_name == 'toshiba 123':
        laptopBuilder = toshibhaBuilder()
    else:
        laptopBuilder = dellBuilder()

    print laptop_name
    client = Client(laptopBuilder)
    laptop = client.getLaptop()
    laptop.specification()
 
  
if __name__ == "__main__":
    laptop_name = 'dell 123'
    main(laptop_name)